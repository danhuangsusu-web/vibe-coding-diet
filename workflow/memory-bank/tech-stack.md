...
当前 `project.config.json` 使用小程序测试号提供的 AppID，已支持真机预览与真机调试，但不能上传代码，因此体验版与上线仍需正式 AppID。正式 AppID 相关工作属于 P1、不计入 P0 预算；D5 决策前不固定“真机体验版”或“真机调试”的最终措辞。P0 不提前引入云数据库、容器平台、反向代理、域名或正式小程序审核流程。
