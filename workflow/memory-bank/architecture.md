...
| `apps/miniprogram/src/services/meal-api.ts` | 小程序请求层：用 `Taro.request` 调用 `__API_BASE_URL__`，统一映射为带 `code` 与 `retryable` 的 `MealApiError`；文字请求与图片上传均返回可 `cancel()` 的任务，底层调用 `abort()`；含评估、记录创建/读取/删除及资料读取/更新方法 |
| `apps/miniprogram/src/services/meal-error-presentation.ts` | 客户端错误呈现层：只接受通过共享 Schema 的错误响应，输出本地稳定文案、恢复动作与可重试标志，不展示堆栈、连接串或供应商正文 |
...
| `apps/miniprogram/src/services/meal-error-presentation.ts` | 把 `MealApiError` 与未知异常统一映射为本地稳定文案加恢复动作（`USE_OFFLINE_SAMPLE`、`RETRY_OR_USE_TEXT`、`EDIT_INPUT`、`RESELECT_IMAGE`、`EDIT_PROFILE`、`RETRY_LATER`、`REFRESH_RECORDS`、`CONFIRM_FALLBACK`） |
