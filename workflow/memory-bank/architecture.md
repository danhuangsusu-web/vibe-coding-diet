...
> 基线日期：2026-09-17（步骤 20 已由用户验收通过；阶段四文字路径完成）
...
当前代码已完成工程骨架、健康接口、模型供应商工厂、shared、nutrition、server 的 Vitest 自动化测试基线、完整的共享业务契约（资料、餐食、评估、记录、统一 API 错误）、按受控词表计算热量区间的规则（含 D1c 的两阶段未知处理）、动态餐次额度与红黄绿灯评级（含高油高糖最低黄灯）、由规则生成的原因与建议白名单、与共享契约对齐的数据库结构和已初始化的固定演示资料、演示资料的读取与更新接口、餐食记录的创建查询与删除接口、两个离线演示样例与统一的 `parseMeal` 解析契约，以及小程序的视觉基础、五页路由、跨页草稿状态和 P01 至 P05 页面。文字输入已通过 `/api/parse-meal` 接入阿里云百炼 OpenAI 兼容模型并完成严格结构校验；图片仍保持本地离线演示，属于步骤 21。

## 2. 根目录职责

| 路径 | 当前职责 | 备注 |
| --- | --- | --- |
| `package.json` | 定义 monorepo 名称、Node/pnpm 约束和两端开发、构建、测试、类型、数据库脚本，并声明根 Vitest 开发依赖 | `test` 运行一次全仓测试，`test:watch` 进入监听模式；`verify:offline-loop` 对运行中的本地服务执行可恢复的真实数据库闭环；`db:seed` 转发到服务端脚本 |
...
| `README.md` | 说明产品、环境、启动、常用检查和当前实施进度 | 文字 AI 验收需设置 `MEAL_PARSER=ai`；图片真实解析仍待步骤 21 |
...
| `apps/miniprogram/src/pages/meal-input/index.tsx` | P02 餐食输入页：单图选择与本地压缩、文字输入、离线样例、提交锁、4 秒进度、20 秒停止等待、错误恢复、草稿写入和前往 P03 |
...
| `apps/miniprogram/src/pages/meal-confirm/index.tsx` | P03 识别确认页：来源摘要、整餐不确定性提示、低可信度置顶的菜品卡列表、增删改菜品、做法与份量编辑、空态引导、草稿写回和前往 P04 |
...
| `apps/miniprogram/src/pages/home/index.tsx` | P01 首页：用 `useDidShow` 拉取记录并在返回时刷新；展示用户区、今日汇总、双入口、当天最近记录、空状态与读取失败态 |
| `apps/miniprogram/src/pages/home/home-records.ts` | P01 的上海日期键、当天记录筛选与倒序、今日看板计算（区间、剩余、封顶进度、超范围判定）、问候语、时间格式化与评级呈现 |
| `apps/miniprogram/src/pages/home/home-records.test.ts` | 覆盖日期键不依赖设备时区、使用服务端 todayDate、当天筛选与排序、看板区间与封顶进度、超范围判定、问候语四时段与三处交界、非法时间回退 |
| `apps/miniprogram/src/pages/history/index.tsx` | P05 历史与设置页：并行读取资料与记录，展示日期分组、汇总区间、演示标记和只读详情入口；支持确认删除、空/加载/失败状态及折叠设置保存 |
| `apps/miniprogram/src/pages/history/index.scss` | P05 日期组、记录卡、占位缩略图、空状态、设置区、步进器、免责声明与窄屏重排样式 |
| `apps/miniprogram/src/pages/history/history-state.ts` | P05 设置草稿构造/校验/请求构造、50 kcal 步进、服务端日期相对标签及删除后的本地汇总重算 |
| `apps/miniprogram/src/pages/history/history-state.test.ts` | 覆盖设置校验与 dirty 状态、步进下限、服务端日期标签、删除后重算和空日期组移除 |
| `apps/miniprogram/src/pages/*/index.scss` | 各页局部样式 |
| `apps/miniprogram/src/pages/*/index.config.ts` | 各页导航标题；按 D6 统一为应用名“食刻 AI” |
| `apps/miniprogram/src/services/meal-api.ts` | 小程序请求层：用 `Taro.request` 调用 `__API_BASE_URL__`，统一映射为带 `code` 与 `retryable` 的 `MealApiError`；含评估、记录创建/读取/删除及资料读取/更新方法 |
...
- P01 至 P05 均已实现并经用户验收；五个页面构成完整的离线闭环；
...
- 小程序请求层已被 P01、P04 与 P05 使用，P05 的资料更新和记录删除分别复用现有 PATCH 与 DELETE 接口；
- TDesign 尚未在业务源码中使用；Jotai 仅用于跨页草稿；
- 五个页面均已在微信开发者工具的模拟器上完成人工验收；真机上的端到端联调受接口地址限制，属于 P1；
- 小程序测试只覆盖纯逻辑（草稿、接口地址、P02/P03/P04/P05 状态、首页与历史记录呈现、离线解析一致性和图片策略），不含页面渲染与微信平台 API；Vitest 范围不包含小程序运行时。
...
| `apps/server/lib/ai-meal-parser.ts` | `text-meal-v2` 文字 Prompt、真实模型调用、20 秒取消、最多 1 次自动重试、受限结构规范化、最终 Zod 校验、稳定错误分型及脱敏 token/成本日志 |
| `apps/server/lib/meal-parse-handlers.ts` | `/api/parse-meal` 的请求校验、模式选择、模型元数据响应头和稳定错误码映射 |
...
文字解析已实现；模型只返回受控食材、做法、份量、置信度和不确定性，不返回热量、评级或健康建议。百炼不兼容 AI SDK 的 JSON Schema 输出参数，因此当前请求普通 JSON 文本，再由服务端执行 `JSON.parse`、受限运输结构校验、确定性规范化和共享 Zod Schema 终检。规范化只补空数组、统一枚举大小写、去重，以及把未受控食材/做法转为 `OTHER` 并保留原标签；额外热量字段、非法份量和没有补充文本的 `OTHER` 仍被拒绝。日志不记录原始输入或供应商响应正文，只记录模型、Prompt、耗时、token、标价成本，以及失败阶段和字段路径。
...
- 前端不再二次计算分组与汇总；
- 响应同时返回 `todayDate`（当天的上海日期键）与 `serverTime`（服务端当前时间），两者都取自 handler 已注入的 `now`。客户端因此不需要读取设备时钟：`todayDate` 决定哪些记录属于今天，`serverTime` 供 P01 生成问候语。这是“服务端是唯一时间权威”在接口层的落地。

P01 的问候语按上海时间取小时：05:00 至 10:59 早上好、11:00 至 13:59 中午好、14:00 至 17:59 下午好、18:00 至次日 04:59 晚上好；无法解析时回退为“你好”。该边界与 nutrition 中用于“剩余餐次数”的 05:00 / 10:30 / 16:00 阈值**完全独立**，按 D3b 不与餐次表混用。
...
| `packages/shared/src/meal.ts` | `textMealParseRequestSchema` 校验仅文字、去首尾空白、长度 1–100 且拒绝额外字段 |
| `apps/miniprogram/src/services/meal-api.ts` | 调用文字解析接口，20 秒超时，并读取解析模式、模型和 Prompt 元数据 |
| `apps/miniprogram/src/services/meal-parser.ts` | 两条明确演示文字和本地图片继续离线；其他文字调用服务端真实模型 |
...
- `ai`：默认使用真实文字 AI 适配器，也允许测试注入；返回结果一律用 `parsedMealSchema` 复验；
- 非法取值抛 `MealParserConfigurationError`。

页面只需调用 `parseMealWithMetadata`，无需自行判断模型模式。两条明确演示文字与本地图片保持离线、免费且标记 `isDemo: true`；其他文字调用服务端并写入真实模型版本、`isDemo: false`，offline 仍是服务端模式开关和演示降级入口。
...
1. 小程序可打开五个页面并形成完整离线闭环：P02 收集单张本地图片或文字并调用离线 `parseMeal`，P03 增删改菜品后确认，P04 调 `POST /api/assess-meal` 取得评估并可用 `POST /api/meal-records` 保存，保存后返回 P01 并自动刷新当天汇总；P05 展示服务端分组历史、支持进入只读详情、确认删除记录及修改演示资料。
...
AI 工厂和文字解析器都已实现；`MEAL_PARSER=ai` 会通过 `/api/parse-meal` 调用唯一配置模型，`offline` 继续匹配两个固定样例。小程序的两条明确演示文字和本地图片不发起模型请求，其他文字统一走服务端。根 Vitest 入口当前可运行 250 个测试，范围覆盖共享契约、nutrition 规则、服务端解析与数据接口、小程序纯逻辑和五页离线数据流；`pnpm verify:offline-loop` 另运行 1 个真实 HTTP/数据库闭环测试，且不会计入普通测试。nutrition 已通过记录接口被真实调用；小程序 P01 至 P05 已完成首页看板、文字 AI/离线图片输入、确认编辑、评估展示、保存、历史删除与设置更新，并已真实调用所需接口。
...
目标链路中的文字路径已实现第 1 至 8 步：P02 收集文字，服务端调用模型并校验结构，P03 允许用户增删改和确认，nutrition 读取资料与当天记录执行规则，服务端保存 MealRecord，小程序随后刷新首页和历史。图片路径目前只完成本地选择、压缩和离线演示，真实上传与视觉模型解析属于步骤 21。P03 本身不评估或保存，而是确认后进入 P04 完成评估与可选保存。

每完成一个计划步骤，必须更新本文件，把对应职责从“目标”改为“当前”。

## 10. 已知技术债与风险

- 普通自动化测试目前是 250 个契约、规则、接口、小程序纯逻辑与跨工作区闭环测试；另有 1 个显式运行、会自动恢复数据的真实 HTTP/数据库闭环测试；尚无针对微信运行时的页面自动化测试；
...
- 文字解析已使用 `AI_NOT_CONFIGURED`、`AI_TIMEOUT`、`AI_INVALID_OUTPUT`、`NO_MEAL_DETECTED` 与 `VALIDATION_FAILED`；图片相关错误码仍待步骤 21 接口使用；
- 小程序已有本地单图选择、最长边 1280px/质量 75 压缩、压缩后 1MB 限制，以及文字请求的 4 秒进度/20 秒停止等待；尚无图片上传和服务端 2MB 请求体限制；
- 文字 AI 已有 JSON、受限运输结构、确定性规范化与最终 Zod 校验，以及配置/超时/非法输出/无餐食/供应商错误分型；`text-meal-v2` 已让“番茄炒鸡蛋”连续两次真实解析成功，步骤 23 仍需正式评测成功率；
- 文字模型当前延迟波动明显：4 次成功约为 6.45 至 16.071 秒，另在 12 秒和 15 秒阈值下观察到超时；验收当日同一句“番茄炒蛋配一小碗米饭”连续三次调用为 20.01 秒超时、20.05 秒超时、12.46 秒成功；当前统一上限为 20 秒，小样本不足以声称正式 P50/P90；
- 没有评测集；
- P01 至 P05 均已通过用户验收；步骤 19 的跨层与真实 HTTP 闭环、两端构建、数据恢复以及多尺寸端到端验收均已通过；步骤 20 的真实文字解析已在微信开发者工具由用户验收通过；
- 开发服务器在依赖或代码变动后可能出现 `.next` 构建缓存失效（表现为任意接口 500 且日志为 `Cannot find module './vendor-chunks/<pkg>.js'`），清空 `apps/server/.next` 并重启即可恢复，与业务代码无关；
