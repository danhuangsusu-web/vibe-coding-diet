...
> 基线日期：2026-09-17（步骤 21 已由用户验收通过；阶段四的文字与图片两条真实解析路径均已完成）
...
| `apps/miniprogram/src/pages/meal-input/index.tsx` | P02 餐食输入页：图片/文字双入口与切换确认、单图选择与本地压缩、图片上传、文字输入、离线样例、提交锁、4 秒进度、20 秒停止等待、错误恢复、草稿写入和前往 P03 |
...
- P02 顶部为“图片 / 文字”双入口，两种输入互斥：切换即清空另一种输入，切到图片且已有文字时先弹确认（D9）；确认弹窗按钮受小程序限制，不得超过 4 个汉字；
- P02 图片在本地选择、预览和压缩后上传到 `/api/parse-meal`，服务端只用于当次识别，不落库、不接入对象存储；两种输入共用同一 `ParsedMeal` 契约；
- P02 只在文字模式下识别两组离线样例；其余文字与所有真实图片都调用服务端 AI 解析，小程序与服务端各自仍保留一份离线样例数据，由一致性测试锁定；
...
| `apps/miniprogram/src/services/meal-api.ts` | 调用文字解析接口并上传图片（字段名 `image`），20 秒超时，读取解析模式、模型和 Prompt 元数据 |
...
目标链路中的文字路径已实现第 1 至 8 步：P02 收集文字，服务端调用模型并校验结构，P03 允许用户增删改和确认，nutrition 读取资料与当天记录执行规则，服务端保存 MealRecord，小程序随后刷新首页和历史。图片路径已完整实现：P02 本地选择、压缩并上传，服务端校验格式与大小后调用视觉模型并复用同一 `ParsedMeal` 契约，之后与文字路径共用 P03 至 P05。P03 本身不评估或保存，而是确认后进入 P04 完成评估与可选保存。

每完成一个计划步骤，必须更新本文件，把对应职责从“目标”改为“当前”。

## 10. 已知技术债与风险

- 普通自动化测试目前是 260 个契约、规则、接口、小程序纯逻辑与跨工作区闭环测试；另有 1 个显式运行、会自动恢复数据的真实 HTTP/数据库闭环测试；尚无针对微信运行时的页面自动化测试；
...
- 没有评测集；
- P01 至 P05 均已通过用户验收；步骤 19 的跨层与真实 HTTP 闭环、两端构建、数据恢复以及多尺寸端到端验收均已通过；步骤 20 的真实文字解析与步骤 21 的真实图片解析均已由用户在微信开发者工具验收通过；图片路径的成功率与菜单截图表现仍待步骤 23 正式评测；
